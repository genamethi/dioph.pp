...
All writes to `primeparts.*` are native C++ tools committing through the catalog
(`MakeLocalCatalog` or `pp-catalogd`). No Python in the commit path.

`generate` is the live producer. It streams prime-ordered batches to
`AlignedBucketWriter`, which writes bucketed parquet for `primes` + `partitions`
to `StagingDataDir`, then commits both tables in one atomic transaction via
`CommitFilesAtomic` (moves staged files into the table tree, one
`commitTransaction`). `--rest-uri` routes through `pp-catalogd`; default is
in-process. `--temp` writes to a scratch warehouse and skips the commit.

General derived-table pattern:

1. Read source files via `SourceTableReader::OpenMetadata` (manifest walk, no fs
   globbing).
2. Stream batches; transform/generate columns; write parquet via the writer.
3. Commit through `CommitFiles(catalog, warehouse, table, schema, spec, files)` —
   ensures the namespace, load-or-creates the table, FastAppends in one snapshot.
   Resume-safe.

### Row-level deletes (covering sieve)

The covering sieve persists progress as merge-on-read position deletes over a
`primes_k0` shallow clone, so the live rows are the current uncovered set
(`pp_row_delta.{h,cc}`; `delete_primitive_spike.md`).

## Reading

### Native (iceberg-cpp) — canonical

`SourceTableReader::OpenMetadata(metadata_path, columns, filter)` walks the
manifest, plans the scan, sorts files by `p` lower-bound, and exposes an Arrow
RecordBatch stream. Do not `recursive_directory_iterator` the data dir: file
order, presence, and paths are manifest-derived.
...
Manifest stats prune by `p_bucket`/`p_min`/`p_max`. Do not `sink_parquet` derived
tables at this scale — polars streaming OOMs on partitioned writes against the
tens-of-billions-row inputs. Produce derived tables natively.

## Schema evolution

1. Edit `PrimesSchema` / `PartitionsSchema` in `native/src/schemas.cc` — add a
   field with the next contiguous `field_id`, type, required flag.
2. Bump the table via `POST .../tables/{table}` with an `UpdateSchema` body, or as
   part of the next binary's `CreateTable`.
3. Never reuse a field_id. Add new ones at the end.
4. New required columns break existing files. Add as nullable, backfill, then
   promote to required in a second commit.
