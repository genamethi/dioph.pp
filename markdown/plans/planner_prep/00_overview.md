...
- sanitizer coverage is our-code-only: `./configure --sanitize` instruments every object primeparts compiles, but vendored static arrow/iceberg are release-built, so ASAN/UBSAN interceptor coverage across the vendor boundary is partial (iceberg exposes ICEBERG_ENABLE_ASAN/UBSAN; arrow has no sanitizer build in the prefix) — filed-by testing_overhaul 05 — owner: unassigned
