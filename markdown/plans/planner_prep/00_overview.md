...
| 05_resume | writer-declared resume state via snapshot summary properties | 01 | done |
...
- live-warehouse snapshots predate the `pp.aligned.*` resume summary keys — `LoadAlignedResume` errors loudly on them; no backfill surface exists (a snapshot summary is immutable, so backfill means a new declared commit) — filed-by 05 rework — owner: unassigned
