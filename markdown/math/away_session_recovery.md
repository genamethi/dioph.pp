...
2⁴+13³`. So "no `n=1` representation" and "graph source" are the same set. This is a
...
## Immediate reduction: n=1 runs are translations, only sums of 2^m survive
...
## A single example - Don't reuse this. Find your own. 
...
`3→137` in the full graph. They **reduce to 6 distinct Hermite expansions**:
...
as the reduction invariant (dense-id the distinct vectors; keep symbolic strings
for inspection). The routing multiplicity is compressible precisely because it
reduces. The store should hold the invariant, and regenerate routing on demand.
