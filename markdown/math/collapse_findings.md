# Chain-collapse: what the exploration established
...
pure translations `t -> t + 2^m`, so a chain collapses to a **canonical word**
`(A0; (n1,C1); …; (nr,Cr))`, where `A0, Ci` are sums of powers of two.

## Findings (measured)

- **Collapse is lossless and large.** At B=10^6: 8.42e12 maximal chains collapse
...
- **137 paradigm reproduced**: 37 chains → 6 collapse classes; `He_1+134`
  (mult 16), `He_4+10·He_2+27` (mult 1, the degree-4 route = `x^4+4x^2+20`).

## Profiling truth (corrects an earlier misattribution)

- Primality (`is_prime_power`) was 0.52s of 130s at 10^6 — **not** the cost, and
  moot anyway since **edges are read from `partitions`, never recomputed**.
- The real cost is the **collapse DP word-merge** (110s in Python) + Hermite
