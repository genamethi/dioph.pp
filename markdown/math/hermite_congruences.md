...
(mod 3)`). So mod p the umbral operator `e^{D²/2}` collapses to the identity at
...
`He_3 = x(x²−3)` has nonzero roots in `F_ℓ` **iff 3 is a quadratic residue mod ℓ**
— verified at **100% agreement over 93 primes** (`He_3` roots ⟺ `(3/ℓ)=1`). The
...
- Characterize `He_n mod ℓ` factorization type for `n ≥ 4` by the higher power
  residues (cubic for n with `3 | gcd(n, ℓ-1)`, etc.) — the exponent-dependent
  filtration primes of the lab-notes l-adic entry.
- Evaluate `He_n(2^{m'}) mod ℓ` along realized chains and test whether the cusp
  (frontier k=0 primes) is distinguished by hitting Hermite roots — the direct
