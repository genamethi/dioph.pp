...
// writes ALL file data (parquet + manifests, via iceberg-cpp's
// FastAppend/Apply) and assembles the per-table (requirements, updates); only
// the catalog head-pointer CAS crosses the seam. Two transports over one
// assembly path: a daemon POST to /v1/transactions/commit, or an in-process
// store transaction.
//
// This TU compiles against iceberg-cpp's internal serde
// (-Ivendor/iceberg-cpp/src), like pp_catalogd.cc, so pp owns the commit
// request end to end instead of going through RestCatalog's opaque single-table
// commit path.
...
/// One table's contribution to an atomic commit: the staged data files to
/// append under `primeparts.<table_name>`. `files` are mutated in place — each
...
/// + Apply() (writes the manifests + manifest list and returns the new
/// snapshot, without committing). The resulting (requirements, updates) for all
/// tables are then committed atomically through exactly one transport:
///   * in-process — when `store` is non-null: `store->RunInTransaction`
///   wrapping
...
/// `*body_json`, WITHOUT committing. This is the daemon transport's request;
/// also used by tests to exercise the server's transaction route directly (e.g.
/// by tampering a requirement to prove atomic rollback). False + `*error` on
/// failure.
